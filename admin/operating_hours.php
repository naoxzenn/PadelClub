...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update_operating_hours') {
    ...
    if ($dayOfWeek < 1 || $dayOfWeek > 7) {
        ...
        // Ensure HH:MM:00
        if (!empty($openTime) && strlen($openTime) === 5) $openTime .= ':00';
        if (!empty($closeTime) && strlen($closeTime) === 5) $closeTime .= ':00';
...
@media (max-width: 640px) {
    ...
    .ophours-table th, .ophours-table td {
    ...
    <div class="container" style="max-width: 100%; padding: 0;">
        ...
        <div style="margin-bottom: 20px; display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 12px;">
            <div>
                ...
                <p style="color: var(--text-muted); font-size: 0.9rem; margin: 0;">Atur jam buka, tutup, dan sesi operasional/istirahat venue PadelClub.</p>
            </div>
        </div>
                    ...
                    <tbody>
                        <?php foreach ($schedule as $dayOfWeek => $row): ?>
                            <tr style="border-bottom: 1px solid var(--border); transition: background 0.15s ease;" onmouseover="this.style.background='rgba(0,0,0,0.015)'" onmouseout="this.style.background='transparent'">
                                ...
                                <td style="padding: 14px 16px; font-family: monospace; font-size: 0.92rem; white-space: nowrap;">
                                    <?php if ($row['is_open']): ?>
                                        <span style="font-weight: 700; color: var(--navy);"><?= substr($row['open_time'], 0, 5) ?></span> – <span style="font-weight: 700; color: var(--navy);"><?= substr($row['close_time'], 0, 5) ?></span> WIB
                                    <?php else: ?>
                                ...
                                <td style="padding: 14px 16px; font-family: monospace; font-size: 0.92rem; white-space: nowrap;">
                                    <?php if ($row['is_open'] && !empty($row['has_shift2'])): ?>
                                        <span style="font-weight: 700; color: var(--blue);"><?= substr($row['open_time_2'], 0, 5) ?></span> – <span style="font-weight: 700; color: var(--blue);"><?= substr($row['close_time_2'], 0, 5) ?></span> WIB
                                    <?php elseif ($row['is_open']): ?>
                                        <span style="color: var(--text-muted); font-size: 0.82rem; font-style: italic;">Nonstop (Satu Sesi)</span>
                                    <?php else: ?>
                                        <span style="color: var(--text-muted);">-</span>
                                    <?php endif; ?>
                                </td>
                                ...
                                <td style="padding: 14px 16px; white-space: nowrap;">
                                    <?php if ($row['is_open']): ?>
                                        <span class="status-confirmed" style="padding: 4px 12px; font-size: 0.76rem; font-weight: 700; border-radius: var(--radius-full); background: rgba(34, 197, 94, 0.15); color: var(--green);">Buka</span>
                                    <?php else: ?>
                                        <span class="status-cancelled" style="padding: 4px 12px; font-size: 0.76rem; font-weight: 700; border-radius: var(--radius-full); background: rgba(239, 68, 68, 0.15); color: #EF4444;">Tutup</span>
                                    <?php endif; ?>
                                </td>
                                ...
                                <td style="padding: 14px 16px; text-align: right; white-space: nowrap;">
                                    <button type="button" class="btn btn-sm btn-outline" 
                                        ...
                                        <span class="material-symbols-outlined" style="font-size: 1rem; vertical-align: middle;">edit</span> Edit
                                    </button>
                                </td>
                            </tr>
                        <?php endforeach; ?>
...
<div class="ophours-modal-backdrop" id="modal-edit-ophours">
    <div class="ophours-modal-dialog">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 18px; border-bottom: 1px solid var(--border); padding-bottom: 12px;">
            ...
            <button type="button" onclick="closeOpModal('modal-edit-ophours')" style="background: none; border: none; font-size: 1.5rem; cursor: pointer; color: var(--text-muted); line-height: 1;">&times;</button>
        </div>
        ...
        <form method="POST" action="">
            ...
            <div class="form-group" style="margin-bottom: 18px;">
                <label style="font-size: 0.85rem; font-weight: 700; color: var(--navy); display: block; margin-bottom: 8px;">Status Operasional</label>
                <div style="display: flex; gap: 20px;">
                    <label style="display: flex; align-items: center; gap: 8px; font-size: 0.9rem; cursor: pointer;">
                        <input type="radio" name="is_open" id="status_open_1" value="1" onchange="toggleOpFormFields(true)">
                        <span style="font-weight: 700; color: var(--green);">Buka</span>
                    </label>
                    <label style="display: flex; align-items: center; gap: 8px; font-size: 0.9rem; cursor: pointer;">
                        <input type="radio" name="is_open" id="status_open_0" value="0" onchange="toggleOpFormFields(false)">
            ...
            <div id="op_time_fields_container">
                <!-- SESI 1 (JAM UTAMA) -->
                <div style="background: rgba(241, 245, 249, 0.6); padding: 14px; border-radius: var(--radius-md); margin-bottom: 16px; border: 1px solid var(--border);">
                    <div style="font-size: 0.82rem; font-weight: 800; color: var(--navy); text-transform: uppercase; letter-spacing: 0.04em; margin-bottom: 10px; display: flex; align-items: center; gap: 6px;">
                        <span class="material-symbols-outlined" style="font-size: 1.1rem; color: var(--blue);">wb_sunny</span> Sesi 1 (Jam Utama)
                    </div>
                    <div class="ophours-grid-2col" style="display: grid; grid-template-columns: 1fr 1fr; gap: 14px;">
                        <div class="form-group" style="margin: 0;">
                            <label for="input_open_time" style="font-size: 0.82rem; font-weight: 700; color: var(--navy); display: block; margin-bottom: 4px;">Jam Buka</label>
                            <input type="time" id="input_open_time" name="open_time" style="width: 100%; padding: 8px 12px; font-size: 0.9rem; border-radius: var(--radius-md); border: 1px solid var(--border);">
                        </div>

                        <div class="form-group" style="margin: 0;">
                            <label for="input_close_time" style="font-size: 0.82rem; font-weight: 700; color: var(--navy); display: block; margin-bottom: 4px;">Jam Tutup / Istirahat</label>
                            <input type="time" id="input_close_time" name="close_time" style="width: 100%; padding: 8px 12px; font-size: 0.9rem; border-radius: var(--radius-md); border: 1px solid var(--border);">
                        </div>
                    </div>
                </div>
                ...
                <div style="margin-bottom: 16px;">
                    <label style="display: flex; align-items: center; gap: 8px; font-size: 0.88rem; font-weight: 700; color: var(--navy); cursor: pointer;">
                        <input type="checkbox" id="chk_enable_shift2" name="enable_shift2" value="1" onchange="toggleShift2Fields(this.checked)">
                        <span>Aktifkan Sesi 2 (Buka Kembali Setelah Istirahat)</span>
                    </label>
                </div>

                <!-- SESI 2 (OPSIONAL AFTER ISTIRAHAT) -->
                <div id="shift2_container" style="background: rgba(238, 242, 255, 0.6); padding: 14px; border-radius: var(--radius-md); margin-bottom: 20px; border: 1px solid rgba(99, 102, 241, 0.2); display: none;">
                    <div style="font-size: 0.82rem; font-weight: 800; color: var(--blue); text-transform: uppercase; letter-spacing: 0.04em; margin-bottom: 10px; display: flex; align-items: center; gap: 6px;">
                        <span class="material-symbols-outlined" style="font-size: 1.1rem; color: var(--blue);">nights_stay</span> Sesi 2 (Setelah Istirahat)
                    </div>
                    <div class="ophours-grid-2col" style="display: grid; grid-template-columns: 1fr 1fr; gap: 14px;">
                        <div class="form-group" style="margin: 0;">
                            <label for="input_open_time_2" style="font-size: 0.82rem; font-weight: 700; color: var(--navy); display: block; margin-bottom: 4px;">Jam Buka Sesi 2</label>
                            <input type="time" id="input_open_time_2" name="open_time_2" style="width: 100%; padding: 8px 12px; font-size: 0.9rem; border-radius: var(--radius-md); border: 1px solid var(--border);">
                        </div>

                        <div class="form-group" style="margin: 0;">
                            <label for="input_close_time_2" style="font-size: 0.82rem; font-weight: 700; color: var(--navy); display: block; margin-bottom: 4px;">Jam Tutup Akhir</label>
                            <input type="time" id="input_close_time_2" name="close_time_2" style="width: 100%; padding: 8px 12px; font-size: 0.9rem; border-radius: var(--radius-md); border: 1px solid var(--border);">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div style="display: flex; justify-content: flex-end; gap: 10px; border-top: 1px solid var(--border); padding-top: 16px;">
                <button type="button" class="btn btn-outline" onclick="closeOpModal('modal-edit-ophours')" style="padding: 9px 18px; font-size: 0.88rem;">Batal</button>
